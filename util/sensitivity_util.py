...
def setup_histogram_loglog_tan(name = "h_2d"):
    ...
    bins_x_n = 80 #small = 20, medium = 40, large = 160
    bin_x_min = 0.1 #np.power(np.tan(np.arcsin(np.sqrt(0.355))),2)
    bin_x_max = 6.0 #np.power(np.tan(np.arcsin(np.sqrt(0.365))),2)
    ...
    bins_y_n = 160 #small = 40, medium = 80, large = 160
    bin_y_min = 1.5e-5
    ...
    h_2d.GetXaxis().SetRangeUser(0.1, 6.0)
    h_2d.GetYaxis().SetRangeUser(1.5, 20+1.5)
    return h_2d
