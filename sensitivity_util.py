...
def get_reactor_info(reactor_list_name):
    '''
    run the reactor_list_selector utility
    '''

    reactor_list_reactors_file = '/home/lidgard/rat-7/data/REACTORS.ratdb'
    reactor_list_status_file = '/home/lidgard/rat-7/data/REACTORS_STATUS.ratdb'
...
def create_reactor_info(output_file, reactor_list_name):
    '''
    write the reactor_list_selector output to file
    '''
    reactor_info = get_reactor_info(reactor_list_name)
...
def create_parameter_info(output_file, h_2d, n_jobs):
    ...
    #sin_2_12s = [h_2d.GetXaxis().GetBinCenter(i+1) for i in range(h_2d.GetNbinsX())]
    tan_12s = [h_2d.GetXaxis().GetBinCenter(i+1) for i in range(h_2d.GetNbinsX())]
    #sin_12s = [h_2d.GetXaxis().GetBinCenter(i+1) for i in range(h_2d.GetNbinsX())]
    
    del_21s = [h_2d.GetYaxis().GetBinCenter(i+1)/y_multiplier for i in range(h_2d.GetNbinsY())]
    sin_13 = 0.02303

    sin_12s = [np.power(np.sin(np.arctan(np.sqrt(i))),2) for i in tan_12s] #convert from tan^2 nin values to sin^2 parameter values
...
def setup_histogram():
    ...
    bins_y_n = 10 #small = 15, medium = 60, large = 240
    bin_y_min = 6.0e-5
    bin_y_max = 9.0e-5
...
def setup_histogram_loglog_tan():
    ...
    bins_x_n = 40 #small = 20, medium = 40, large = 160
    bin_x_min = 0.1
    bin_x_max = 6
